...
class CapcutConverter:
    ...
    def process_file(self, input_file: str, filename: str, settings_paths: dict = None) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            # 使用設定檔的路徑或預設路徑
            if settings_paths and 'txt_1A' in settings_paths:
                output_dir = Path(settings_paths['txt_1A'])
            else:
                # 預設路徑：主程式目錄 / txt / 1A
                output_dir = Path(os.getcwd()) / 'txt' / '1A'
