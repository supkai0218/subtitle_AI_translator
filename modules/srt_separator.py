...
class SrtSeparator:
    ...
    def convert(self, output_dir, base_filename, input_srt_path=None):
        """
        ...
        default_dir = os.path.join(current_dir, "srt", "in")
        resolved_input_path = None

        if input_srt_path:
            candidate_path = os.path.abspath(input_srt_path)
            if os.path.exists(candidate_path):
                resolved_input_path = candidate_path
            else:
                print(f"[SrtSeparator] WARN 指定的 input_srt_path 不存在: {candidate_path}")

        if resolved_input_path is None:
            resolved_input_path = os.path.join(default_dir, f"{base_filename}.srt")

        print(
            f"[SrtSeparator] DEBUG base_filename={base_filename}, cwd={current_dir}, 預設輸入資料夾={default_dir}, "
            f"實際使用路徑={resolved_input_path}, 輸出目錄={output_dir}"
        )
        
        if not os.path.exists(resolved_input_path):
            existing_files = os.listdir(default_dir) if os.path.exists(default_dir) else "資料夾不存在"
            print(f"[SrtSeparator] DEBUG 找不到輸入 SRT，srt/in 現有檔案: {existing_files}")
            raise FileNotFoundError(f"找不到 SRT 檔案：{resolved_input_path}")
        
        timecode_dict, subtitle_dict = self.parse_srt(resolved_input_path)
