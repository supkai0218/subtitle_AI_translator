...
class SensitiveWordReplacer:
    """
    敏感詞取代器 v2.0
    - v2.0: 支援從settings.json讀取路徑配置，修復路徑不一致問題
    - v1.0: 修改 process_files 方法以接受動態的 input_path，解決路徑寫死問題。
    """
    def __init__(self, filename: str, progress_callback: Optional[Callable[[int, str], None]] = None, settings_paths: Optional[dict] = None):
        """
        初始化 SensitiveWordReplacer。
        
        :param filename: 檔案名稱（用於決定輸出路徑）
        :param progress_callback: 進度回調函數，格式為 (progress: int, message: str) -> None
        :param settings_paths: 從settings.json讀取的路徑配置
        """
        self.filename = filename  # 傳入的檔案名稱
        self.settings_paths = settings_paths  # 設定路徑配置
        self.progress_callback = progress_callback  # 進度回調函數
        
        # 使用設定路徑或預設路徑
        if self.settings_paths and 'markers_db' in self.settings_paths:
            self.db_path = Path(self.settings_paths['markers_db'])
        else:
            # 預設路徑：主程式目錄 / json / markers_db.json
            self.db_path = Path(os.getcwd()) / 'json' / 'markers_db.json'
        
        if self.settings_paths and 'txt_1C' in self.settings_paths:
            output_dir = Path(self.settings_paths['txt_1C'])
        else:
            # 預設路徑：主程式目錄 / txt / 1C
            output_dir = Path(os.getcwd()) / 'txt' / '1C'
        
        self.output_path = output_dir / f"1C-txt_{self.filename}.txt"  # 輸出文字檔路徑
