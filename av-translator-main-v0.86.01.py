#V0.86.01 AI翻譯並行處理與驗證機制 AI翻譯優化測試版
import sys
import os
import json
import shutil
...
from modules.capcut_converter_v02 import CapcutConverter
from modules.text_filter_v01 import TextFilter
from modules.text_marker_v01 import SensitiveWordReplacer
...
class SettingsDialog(QDialog):
    ...
    def setup_paths_tab(self, tab):
        ...
        file_keys = ["markers_db", "script_2A", "script_2B", "script_1B_filter"]
...
class ProcessWorker(QThread):
    ...
    def run_1A_parse(self):
        if self.source_mode == "capcut":
            ...
            success, msg = converter.process_file(self.input_file, self.output_filename)
        else:
    ...
    def run_1C_mark(self, from_stage='1B'):
        ...
        marker = SensitiveWordReplacer(filename=self.output_filename)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("字幕AI翻譯系統 v0.86.01")
        self.resize(700, 750)
        self.output_filename = None
